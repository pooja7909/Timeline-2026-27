...
export const StudentView: React.FC<StudentViewProps> = ({
  ...
  plan.forEach(term => {
    term.rows.forEach(row => {
      if (row.kind === 'week') {
        ...
        if (cell && (cell.assess || cell.text.toLowerCase().includes('assessment') || cell.text.toLowerCase().includes('exam') || cell.text.toLowerCase().includes('test') || cell.text.toLowerCase().includes('mock'))) {
          allAssessments.push({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      {/* Selected Year Badge & Key Info Card */}
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div 
            className="w-12 h-12 rounded-2xl flex items-center justify-center font-display font-black text-xl text-white shadow-sm"
            style={{ backgroundColor: selectedYear.color }}
          >
            {selectedYear.short}
          </div>
          <div>
        ...
        {plan
          ...
          .map(term => {
            const rows = term.rows.filter(row => {
              ...
              if (filterAssessmentsOnly && !cell.assess && !cell.text.toLowerCase().includes('assessment') && !cell.text.toLowerCase().includes('exam')) {
                return false;
              }
              if (search) {
              ...
              <div key={term.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-100">
                  {rows.map((row, rIdx) => {
                    ...
                    const hasAssess = cell.assess || cell.text.toLowerCase().includes('assessment') || cell.text.toLowerCase().includes('exam');
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <div className="flex items-start gap-2">
                            {hasAssess && (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-rose-100 text-rose-800 border border-rose-200 text-xs font-bold font-mono-code flex-shrink-0 mt-0.5">
                                <Star className="w-3 h-3 fill-current text-rose-600" />
                                Assessment
                              </span>
                            )}
                            <div className={`text-base font-semibold ${
                              ...
                              {cell.text || <span className="text-slate-400 italic">Curriculum topic to be announced</span>}
                            </div>
                          </div>
                        </div>
